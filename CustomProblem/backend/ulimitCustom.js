...
const submitSolution = async (problemId, solutionFile) => {
    ...
    const memoryLimit = parseLimit(problem.memoryLimit); // Memory limit in bytes

    const runPromise = new Promise((resolve, reject) => {
        const child = exec(`ulimit -v ${memoryLimit} && ${sourceFilePath}.out`, {
    ...
    try {
        ...
        if (runError.message === 'Time Limit Exceeded') {
            ...
            console.error(`Execution exceeded memory limit: ${runError.message}`);
